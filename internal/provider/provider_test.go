...
func TestWriteRemoteFile(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		call := mock.writeCalls[0]
		if call.path != "/path/file.txt" {
			t.Errorf("path = %q, want %q", call.path, "/path/file.txt")
		}
		if string(call.content) != "content" {
			t.Errorf("content = %q, want %q", call.content, "content")
		}
		if call.fileMode != 0644 {
			t.Errorf("fileMode = %04o, want %04o", call.fileMode, os.FileMode(0644))
		}
		if call.dirMode != 0755 {
			t.Errorf("dirMode = %04o, want %04o", call.dirMode, os.FileMode(0755))
		}
	})
...
func TestReadRemoteFile(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		if len(mock.readCalls) != 1 || mock.readCalls[0] != "/path/file.txt" {
			t.Errorf("readCalls = %v, want [/path/file.txt]", mock.readCalls)
		}
	})
...
func TestDeleteRemoteFile(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		...
		if len(mock.deleteCalls) != 1 || mock.deleteCalls[0] != "/path/file.txt" {
			t.Errorf("deleteCalls = %v, want [/path/file.txt]", mock.deleteCalls)
		}
	})
...
func TestGetRemoteFileInfo(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		expected := &remote.FileInfo{Mode: 0644, Size: 42}
		...
		if info.Mode != 0644 {
			t.Errorf("Mode = %04o, want %04o", info.Mode, os.FileMode(0644))
		}
		if info.Size != 42 {
...
func TestChmodRemoteFile(t *testing.T) {
	config := &scpProviderConfig{Host: "test"}
	...
	t.Run("success", func(t *testing.T) {
		...
		if len(mock.chmodCalls) != 1 {
			t.Fatalf("expected 1 chmod call, got %d", len(mock.chmodCalls))
		}
		if mock.chmodCalls[0].path != "/path/file.txt" {
			t.Errorf("path = %q, want %q", mock.chmodCalls[0].path, "/path/file.txt")
		}
		if mock.chmodCalls[0].mode != 0600 {
			t.Errorf("mode = %04o, want %04o", mock.chmodCalls[0].mode, os.FileMode(0600))
		}
	})
...
func (m *mockClient) FileExists(remotePath string) (bool, error) {
	return m.existsResult, m.existsErr
}
func (m *mockClient) GetFileInfo(remotePath string) (*remote.FileInfo, error) {
	return m.fileInfo, m.fileInfoErr
}

func (m *mockClient) WriteFile(remotePath string, content []byte, fileMode os.FileMode, dirMode os.FileMode) error {
	m.writeCalls = append(m.writeCalls, writeCall{remotePath, content, fileMode, dirMode})
	return m.writeErr
}

func (m *mockClient) ReadFile(remotePath string) ([]byte, error) {
	m.readCalls = append(m.readCalls, remotePath)
	return m.readResult, m.readErr
}

func (m *mockClient) DeleteFile(remotePath string) error {
	m.deleteCalls = append(m.deleteCalls, remotePath)
	return m.deleteErr
}

func (m *mockClient) Chmod(remotePath string, mode os.FileMode) error {
	m.chmodCalls = append(m.chmodCalls, chmodCall{remotePath, mode})
	return m.chmodErr
}
...
func withMockClient(t *testing.T, mock *mockClient) {
	...
	newClientFunc = func(_ *remote.Config) (remote.Client, error) {
		return mock, nil
	}
	t.Cleanup(func() { newClientFunc = original })
}

func withMockCreateClientFailure(t *testing.T) {
	t.Helper()
	original := newClientFunc
	newClientFunc = func(_ *remote.Config) (remote.Client, error) {
		return nil, fmt.Errorf("create client failure")
	}
	t.Cleanup(func() { newClientFunc = original })
}
